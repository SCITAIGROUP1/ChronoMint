  ...
  it("rejects update when project is inactive", async () => {
    ...
    await expect(
      ...
      (err: unknown) =>
        ...
        err.code === ErrorCodes.TIMELOG_NOT_EDITABLE &&
        err.message.includes("inactive projects")
    );
  });

  it("rejects update when category is inactive", async () => {
    mockPrisma.timeLog.findFirst.mockResolvedValue({
      ...baseLog,
      task: { ...baseLog.task, category: { isActive: false } }
    });
    ...
    await expect(
      ...
      (err: unknown) =>
        err instanceof DomainException &&
        err.code === ErrorCodes.TIMELOG_NOT_EDITABLE &&
        err.message.includes("inactive categories")
    );
  });
  ...
  it("rejects delete when task is inactive", async () => {
    ...
    await expect(service.remove("ws-1", "user-1", "MEMBER", "log-1")).rejects.toSatisfy(
      (err: unknown) =>
        err instanceof DomainException &&
        err.code === ErrorCodes.TIMELOG_NOT_EDITABLE &&
        err.message.includes("inactive tasks")
    );
  });
});
