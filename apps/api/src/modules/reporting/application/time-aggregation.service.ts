...
export type TimeLogWithRelations = Awaited<
  ReturnType<TimeAggregationService["fetchLogs"]>
>[number];
...
export class TimeAggregationService {
  ...
  buildAggregates(
    ...
    const byUser = new Map<
      string,
      HoursAgg & { userName: string; userEmail: string }
    >();
    const daily = new Map<
      string,
      Map<
...
export function roundExport(n: number) {
