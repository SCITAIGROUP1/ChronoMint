...
export class CustomThrottlerGuard extends ThrottlerGuard {
  protected override async handleRequest(requestProps: ThrottlerRequest): Promise<boolean> {
    ...
    if (throttler.name === "auth") {
      ...
      const hasLimitOverride = this.reflector.getAllAndOverride(`THROTTLER:LIMIT${throttler.name}`, [handler, classRef]);
      const hasTtlOverride = this.reflector.getAllAndOverride(`THROTTLER:TTL${throttler.name}`, [handler, classRef]);
