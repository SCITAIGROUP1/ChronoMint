...
export function BudgetBurnDownWidget({ projectId }: { projectId?: string }) {
  ...
  if (projectId && data) {
    ...
    const statusConfigs = {
      no_budget: { label: "No Budget", color: "text-muted-foreground bg-muted border-muted-foreground/15", icon: Info },
      on_track: { label: "On Track", color: "text-green-700 bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-900/30", icon: CheckCircle },
      near_budget: { label: "Near Budget (>90%)", color: "text-amber-700 bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/30", icon: AlertTriangle },
      over_budget: { label: "Over Budget", color: "text-red-700 bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-900/30", icon: AlertTriangle }
    };
    ...
    return (
      <Card className="transition-all duration-300 hover:shadow-md">
        <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
          ...
          <span className={`text-xs px-2 py-0.5 rounded-full border font-medium flex items-center gap-1 ${config.color}`}>
        ...
        <CardContent className="space-y-4 pt-2">
          <div className="flex justify-between items-baseline">
            <div>
              <p className="text-2xl font-bold tracking-tight">
                ...
                <span className="text-sm font-medium text-muted-foreground">/ {budgetHours} hrs</span>
              </p>
            ...
            {percentUsed !== null && (
              <span className={`text-xl font-bold ${status === "over_budget" ? "text-destructive" : status === "near_budget" ? "text-amber-500" : "text-primary"}`}>
                {percentUsed}%
              </span>
            )}
          </div>
          ...
          <div className="w-full bg-muted/40 rounded-full h-2 overflow-hidden border">
            <div
              className={`h-full transition-all duration-500 ${
                status === "over_budget" ? "bg-red-500" : status === "near_budget" ? "bg-amber-500" : "bg-primary"
              }`}
          ...
          {status !== "over_budget" && percentUsed && percentUsed > 0 && (
            <p className="text-xs text-muted-foreground leading-relaxed flex items-center gap-1.5">
              ...
              Project is trending {status === "near_budget" ? "close to" : "on track for"} its budget capacity.
            </p>
          )}
        </CardContent>
      </Card>
