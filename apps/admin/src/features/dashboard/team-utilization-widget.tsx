...
import { Card, CardContent, CardHeader, CardTitle, Table, TableHeader, TableRow, TableHead, TableBody, TableCell } from "@chronomint/ui";
...
export function TeamUtilizationWidget({ from, to }: { from: string; to: string }) {
  ...
  const statusConfigs = {
    on_track: { label: "On Track", color: "text-green-700 bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-900/30", icon: CheckCircle2 },
    low: { label: "Low", color: "text-amber-700 bg-amber-50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/30", icon: AlertCircle },
    critical: { label: "Critical", color: "text-red-700 bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-900/30", icon: AlertCircle }
  };
                ...
                {data.members.map((m) => {
                  ...
                  return (
                    <TableRow key={m.userId} className="hover:bg-muted/30">
                      ...
                      <TableCell className="text-right text-xs py-2 font-mono">{m.loggedHours.toFixed(1)}h</TableCell>
                      <TableCell className="text-right text-xs py-2 text-muted-foreground font-mono">{m.billableHours.toFixed(1)}h</TableCell>
                      <TableCell className="text-right text-xs py-2 font-bold font-mono">{m.utilizationPct}%</TableCell>
                      <TableCell className="py-2 text-center">
                        <span className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border font-medium ${config.color}`}>
