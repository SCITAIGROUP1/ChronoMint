...
export async function clearAppBrowserSession(page: Page) {
  await page.goto("/login");
  await page.evaluate(() => {
    for (const key of [
      "cm-app-access-token",
      "cm-app-workspace-id",
      "cm-app-refresh-token",
      "cm-access-token",
      "cm-workspace-id",
      "cm-refresh-token"
    ]) {
...
export async function loginAsMember(page: Page) {
  await clearAppBrowserSession(page);
  await page.goto("/login");
  await submitLoginForm(page, MEMBER_EMAIL, SEED.personas.individualContributor.password);
}
