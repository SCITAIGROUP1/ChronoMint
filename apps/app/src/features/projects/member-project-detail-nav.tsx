...
export function buildMemberProjectDetailNavItems(projectId: string): MemberProjectDetailNavItem[] {
  return DEFAULT_ITEMS.map((item) => ({
    ...item,
    href: `/my-projects/${projectId}/${item.id}`
  }));
}

/** Landing route when opening a project from My Projects. */
export function myProjectDetailHref(projectId: string): string {
  return `/my-projects/${projectId}/overview`;
}
