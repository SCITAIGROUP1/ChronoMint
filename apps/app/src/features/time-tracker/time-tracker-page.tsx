...
export function AdminTimeTrackerPage({
  ...
  const projects = useMemo(() => {
    if (!managedProjectIds) return catalog.projects;
    const allowed = new Set(managedProjectIds);
    return catalog.projects.filter((project) => allowed.has(project.id));
  }, [catalog.projects, managedProjectIds]);
  const tasks = useMemo(() => {
    if (!managedProjectIds) return catalog.tasks;
    const allowed = new Set(managedProjectIds);
    return catalog.tasks.filter((task) => allowed.has(task.projectId));
  }, [catalog.tasks, managedProjectIds]);
  ...
  useEffect(() => {
    if (!ws || managedProjectIds) return;
  ...
  }, [managedProjectIds, ws]);

  useEffect(() => {
    const scopedProjectIds =
      projectFilter.length > 0 ? projectFilter : managedProjectIds ? [...managedProjectIds] : [];
    if (!ws || scopedProjectIds.length === 0) {
      setProjectMembers([]);
      return;
    }

    Promise.all(
      scopedProjectIds.map((id) =>
  ...
  }, [managedProjectIds, ws, projectFilter]);

  const members = useMemo(() => {
    return managedProjectIds || projectFilter.length > 0 ? projectMembers : allMembers;
  }, [managedProjectIds, projectFilter, projectMembers, allMembers]);
  ...
  const memberMap = useMemo(
    () =>
      new Map((managedProjectIds ? projectMembers : allMembers).map((m) => [m.userId, m.userName])),
    [allMembers, managedProjectIds, projectMembers]
  );
