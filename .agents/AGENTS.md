...
- **Fallback**: If global or turbo-level test/build scripts fail due to environment issues (e.g., `pnpm` not found in turbo), you must CD directly into the specific package/app directory (e.g., `cd apps/client`) and run the localized compiler (`tsc --noEmit` or `vitest`) to verify the integrity of your changes before pushing.
