"use client";

import { cn } from "@kloqra/ui";
import { ChevronRight } from "lucide-react";
import Link from "next/link";
import type { AdminContextMode } from "../auth/admin-context";
import { resolveAdminContextBreadcrumb } from "../auth/admin-context";
import { useTenantCurrent } from "../features/tenant/use-tenant-current";
import { useSessionStore } from "../stores/session.store";

export type AdminContextBreadcrumbProps = {
  contextMode: AdminContextMode;
  className?: string;
};

export function AdminContextBreadcrumb({ contextMode, className }: AdminContextBreadcrumbProps) {
  const session = useSessionStore((s) => s.session);
  const { tenant } = useTenantCurrent();

  const segments = resolveAdminContextBreadcrumb({
    session,
    tenantName: tenant?.name,
    contextMode
  });

  if (segments.length === 0) return null;

  return (
    <nav
      aria-label="Current context"
      className={cn(
        "sticky top-0 z-20 -mx-4 border-b border-border/70 bg-background/95 px-4 py-2.5 text-xs text-muted-foreground backdrop-blur-md @min-[1101px]/shell:-mx-6 @min-[1101px]/shell:px-6",
        className
      )}
    >
      <ol className="flex min-w-0 flex-wrap items-center gap-1">
        {segments.map((segment, index) => {
          const isLast = index === segments.length - 1;

          return (
            <li key={`${segment.label}-${index}`} className="flex min-w-0 items-center gap-1">
              {index > 0 ? (
                <ChevronRight className="size-3 shrink-0 text-muted-foreground/70" aria-hidden />
              ) : null}
              {segment.href && !isLast ? (
                <Link
                  href={segment.href}
                  className="truncate font-medium text-muted-foreground transition-colors hover:text-foreground"
                >
                  {segment.label}
                </Link>
              ) : (
                <span
                  className={cn(
                    "truncate",
                    isLast ? "font-medium text-foreground" : "text-muted-foreground"
                  )}
                  aria-current={isLast ? "page" : undefined}
                >
                  {segment.label}
                </span>
              )}
            </li>
          );
        })}
      </ol>
    </nav>
  );
}
