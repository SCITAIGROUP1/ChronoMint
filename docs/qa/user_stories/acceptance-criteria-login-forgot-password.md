...
> Derived from `docs/architecture/AUTH.md`, `docs/architecture/MULTI_DEVICE_SESSIONS.md`, `docs/architecture/TENANT_RBAC.md`, `docs/architecture/CONTEXT.md`, `docs/specs/auth-workspace.md`, `docs/specs/user-profile.md`, the current implementation (`apps/api/src/modules/auth/`, `packages/contracts/src/dto/auth.dto.ts`, `packages/contracts/src/dto/common.dto.ts`), and GitHub issues [#561 "[Team Member \| Authentication] Team Member Login"](https://github.com/SCITAIGROUP1/ChronoMint/issues/561) and [#562 "[Team Member \| Workspace Selection] Select Workspace After Login"](https://github.com/SCITAIGROUP1/ChronoMint/issues/562) on the [Kloqra Project board](https://github.com/orgs/SCITAIGROUP1/projects/6/views/1).
...
### AC-14: Login screen composition (GitHub #561)

- **Given** a signed-out user opens the Login page
- **Then** the screen presents: an Email Address field, a Password field, a primary sign-in submit button, and a "Forgot password?" link
- **Note (RQA #561, verified 2026-07-01):** the ticket specifies the button label as "Login"; the current implementation labels it "Sign in". Functionally equivalent — flagged as a copy discrepancy, not a functional defect.

### AC-15: Required-field validation on empty submit (GitHub #561)

- **Given** the Email and/or Password field is left blank
- **When** the form is submitted
- **Then** each empty field shows its own inline required-field error, and no `POST /auth/login` request is sent
- **Note (RQA #561, verified 2026-07-01):** the ticket specifies the literal wording "This field is required" per field; the current implementation shows field-specific wording ("Email is required" / "Password is required"). Wording difference only — validation itself is present and functions correctly.

### AC-16: Authenticated users redirected away from the Login page (GitHub #561)

- **Given** a user with an active, valid session
- **When** they navigate directly to the Login page (e.g., via back button or a saved link)
- **Then** they are redirected away to their workspace rather than shown the login form again
- **Note (RQA #561, verified 2026-07-01):** confirmed **partially implemented only** — protected workspace routes correctly redirect an *unauthenticated* user to `/login`, but the Login page itself does not check for an existing session and redirect an *already-authenticated* user away from it. Tracked as a known gap — see "Out of scope / not yet implemented".

### AC-17: Post-login workspace redirect (GitHub #561 AC3, #562 AC1 & AC4)

- **Given** a successful login (AC-1)
- **When** the member belongs to **more than one** workspace
- **Then** they are routed to the Workspace Selection screen
- **When** the member belongs to **exactly one** workspace
- **Then** the Workspace Selection screen is bypassed entirely and the member is routed directly to their startup page (default `/timer`, or the page set in Account Preferences)

### AC-18: Workspace Selection screen — card display (GitHub #562 AC2)

- **Given** the Workspace Selection screen is shown (AC-17)
- **Then** each workspace is displayed as a selectable card showing the Workspace Name, a logo/avatar (initials-based, gradient-colored), and the member's role badge in that workspace
- **Note (RQA #562, verified 2026-07-01):** the ticket also specifies a "Number of Projects" count per card; at initial QA this was **not implemented** (cards showed name, avatar, and role only). A defect was filed and has since been resolved (GitHub #644, "Workspace switcher missing project count per #562" — status: Completed). Re-verify project count is present on next regression pass.

### AC-19: Selecting a workspace (GitHub #562 AC3)

- **Given** the Workspace Selection screen
- **When** the member clicks a workspace card
- **Then** that workspace is set as the active session workspace (`POST /auth/switch-workspace`) and the member is redirected to their main working screen

### AC-20: Switching workspaces without logging out (GitHub #562 AC5)

- **Given** a member is signed in
- **When** they open the workspace switcher and select a different workspace
- **Then** the active workspace is switched in place (no logout/re-login required)
- **Note (RQA #562, verified 2026-07-01):** confirmed working via the workspace switcher in the left sidebar navigation. The ticket describes it as living in the "top" navigation bar — placement differs from the story wording, but the functionality itself matches.

---

## 2. Forgot Password Module

### AC-21: Request a password reset link
...
### AC-22: Rate limiting on forgot-password requests
...
### AC-23: Reset password with a valid token
...
### AC-24: Reset password with an invalid or expired token
...
### AC-25: Reset token is single-use

- **Given** a token that was already successfully used to reset a password
- **When** the same token is submitted again
- **Then** the request fails as invalid/expired (AC-24), since the token hash was cleared on first use

### AC-26: New password must meet policy
...
### AC-27: Rate limiting on reset-password requests
...
### AC-28: Platform (superadmin) scope isolation
...
### AC-29: No session issued directly from reset
...
## Known gaps vs. ticket wording (tracked, not re-tested here)

- **Authenticated-user redirect on the Login page itself** (AC-16) — only enforced at the protected-route level today, not on `/login` directly. Confirmed via RQA #561 (2026-07-01).
- **Copy wording differences** (AC-14, AC-15) — submit button reads "Sign in" not "Login"; empty-field errors read "Email/Password is required" not the literal "This field is required" from the ticket. Both are functionally correct; flagged for copy alignment only. Confirmed via RQA #561 (2026-07-01).
