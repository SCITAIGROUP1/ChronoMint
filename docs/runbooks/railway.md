...
| `FATAL: DATABASE_URL is empty` / immediate crash | `docker-entrypoint.sh` exits when the API container has no `DATABASE_URL`. On the **API** service (not Postgres), add `DATABASE_URL` via **Reference** from your Postgres plugin (`${{YourDbServiceName.DATABASE_URL}}` — name is case-sensitive), or paste the full `postgresql://` URL. **Save variables, then redeploy the API** (a running replica does not pick up new vars). Success logs show `DATABASE_URL is set (N characters)` then `Running prisma migrate deploy...`. |
