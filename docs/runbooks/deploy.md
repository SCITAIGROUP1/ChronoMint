...
Kloqra deploys one customer-facing product from `apps/client`. The internal `apps/platform-admin`
...
| Product (`apps/client`) | Vercel                  | [`apps/client/vercel.json`](../../apps/client/vercel.json) |
...
4. Deploy `apps/client` with `NEXT_PUBLIC_AUTH_SCOPE=app`.
...
| ----------- | ---------------- | -------------------- | ------------- |
| Staging     | `kloqra-staging` | `kloqra-app-staging` | `apps/client` |
| Production  | `kloqra-prod`    | `kloqra-app`         | `apps/client` |
...
If the product deployment is unhealthy, revert `apps/client` to its previous Vercel deployment. API
