...
5. Implement customer workflows in `apps/client` using shared packages.
...
| BE   | `apps/api/src`                                      |
| FE   | `apps/client`, `packages/ui`, `packages/web-shared` |
| QA   | tests and e2e coverage                              |
| LSA  | contracts and orchestration                         |

All customer personas use `apps/client` with auth scope `app`. `apps/platform-admin` remains isolated.
