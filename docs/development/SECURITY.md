...
`PUBLIC_APP_URL` and `PUBLIC_PLATFORM_URL` must list exact frontend origins. The API enables
credentials for cookie-based refresh.
...
**Startup gates:** Production API refuses to start without `PUBLIC_APP_URL` or
`PUBLIC_PLATFORM_URL`, with wrong cookie SameSite for cross-site Vercel+Railway, or with
`COOKIE_DOMAIN` set in that topology.

## RBAC

The customer frontend is one capability-driven product, but the API remains the authorization
boundary. The central policy evaluator combines tenant, workspace, project, and self scopes;
route visibility and capability snapshots are presentation hints only.

Workspace and project role changes are audited and write a user-revocation timestamp. Access tokens
issued before the change fail immediately with `session_revoked`; role changes do not delete the
user's refresh-family rows. The product clears stale local role/capability state and requires a new
sign-in. This avoids waiting for access-token expiry after either a promotion or demotion.
...
- Members do not receive workspace-wide revenue or other members’ hours from the API or unified
  product by design.
...
2. Review `PUBLIC_APP_URL` and `PUBLIC_PLATFORM_URL` for unexpected domains.
